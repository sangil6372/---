            ...
            if match:
                ...
                if page_number < len(pdf_document):
                    ...
                    try:
                        ...
                        print(f"Error: JSON file {json_file} is not valid JSON.")
                        continue
                    ...
                    # 이미지 크기 비율 계산
                    width_ratio = page.rect.width / image_width
                    height_ratio = page.rect.height / image_height
                    ...
                    for shape in shapes:
                        if shape['label'] == 'IMAGE':
                            ...
                            text_latex = page.get_text("text",
                                                       clip=(x0_points, y0_points, x1_points, y1_points)).strip()
                            ...
                            for inner_shape in shapes:
                                if inner_shape["label"] == "FORMULA":
                                    fx0, fy0 = inner_shape["points"][0]
                                    fx1, fy1 = inner_shape["points"][1]
                                    ...
                                    is_formula_in_textbox = (
                                            x0_points <= mid_x <= x1_points and
                                            y0_points <= mid_y <= y1_points
                                    )

                                    # 수식 박스 중심 점이 텍스 박스 안에 있으면 안에 포함된 것으로 판단
                                    if is_formula_in_textbox:
                                        ...
                                        words = page.get_text("words",
                                                              clip=(x0_points, y0_points, x1_points, y1_points))
