...
def is_inline_formula(shape, text_bboxes):
    ...
    for tx1, ty1, tx2, ty2 in text_bboxes:
        ...
        if left <= cx <= right and top <= cy <= bottom: # 내부가 하나라도 있으면 true
             return True
...
def check_and_save_json_files(folder_list, output_base_folder):
    ...
    output_folder = os.path.join(output_base_folder, f"output_{group_number}")
            ...
            for filename in files:
                ...
                if filename.endswith(".json"):
                    ...
                    with open(file_path, 'r', encoding='utf-8') as file:
                        ...
                        found_formula = any(
                            shape['label'] == 'FORMULA' and not is_inline_formula(shape, text_bboxes)
                        ...
                        if not found_table and found_formula:
                            ...
                            if file_count >= 2000:
                                group_number += 1
                                output_folder = os.path.join(output_base_folder, f"output{group_number}")
                                if not os.path.exists(output_folder):
                                    os.makedirs(output_folder)
                                file_count = 0
